...
const AppLayout = ({ children }: AppLayoutProps) => {
  ...
  return (
    <div className="min-h-screen flex w-full bg-background">
      <aside
        className={`sticky top-0 h-screen border-r border-border bg-background flex flex-col transition-all duration-300 ${
          collapsed ? "w-16" : "w-56"
        }`}
      >
        <div className="flex items-center gap-2 px-4 h-14 border-b border-border shrink-0">
          ...
          {!collapsed && (
            <span className="font-bold text-sm tracking-tight">SQL Quest</span>
          )}
        </div>
